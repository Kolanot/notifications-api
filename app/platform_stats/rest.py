...
def get_usage_for_all_services():
    ...
    for service in billing_details:
        if service.service_id in combined:
            combined[entry.service_id].update({
                'purchase_order': service.purchase_order,
                'billing_contact_names': service.billing_contact_names,
                'billing_contact_email_addresses': service.billing_contact_email_addresses,
